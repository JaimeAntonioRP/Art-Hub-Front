...
// ---------- Inline SVG icon set (Lucide-style, hand-picked) ----------
const ICON_PATHS = {
  ...
  const cards = [
    {
      ...
      title: "Certificación biométrica de cada pincelada.",
      body:
        "Una red neuronal entrenada con más de 38,000 obras del corpus cusqueño extrae la firma biométrica del óleo —textura, ritmo del trazo, capa pigmentaria— generando un certificado único e irrepetible.",
      bullets: ["Análisis multiespectral", "Firma de trazo", "Detección de retoque"],
    },
    {
      num: "02",
      icon: "shield-check",
      kicker: "Blockchain",
      title: "Título de propiedad inmutable y trazable.",
      body:
        "Cada obra recibe un token de procedencia anclado a una cadena pública. Transferencias, exhibiciones y restauraciones quedan registradas en un historial criptográfico verificable durante generaciones.",
      bullets: ["Smart-contract custodiado", "Historial perpetuo", "Royalties al maestro"],
    },
    {
      num: "03",
      icon: "package-check",
      kicker: "Logística llave en mano",
      title: "Exportación, seguro y entrega global.",
      body:
        "Alianza operativa con DHL Fine Art: embalaje museístico, trámite CITES, póliza total y trazabilidad sensórica desde el taller del maestro hasta la residencia o bóveda del coleccionista.",
      bullets: ["Embalaje museístico", "Trámite documental", "Cobertura puerta a puerta"],
    },
  ];
...
const WORKS = [
  {
    lot: "Lote 027",
    title: "Virgen del Carmen (estudio)",
    master: "Maestro Edilberto Mérida",
    origin: "Cusco · 2025",
    medium: "Óleo sobre lienzo · 80 × 60 cm",
    price: "USD 15,000",
    tags: ["Validado por IA", "AR Disponible", "Procedencia 0x4a…b1"],
    palette: ["#3a2a1f", "#7a4a30", "#c89058", "#e9d4a8"],
  },
  {
    lot: "Lote 031",
    title: "Plaza de Armas, lluvia tardía",
    master: "Maestro Antonio Olave",
    origin: "Cusco · 2024",
    medium: "Óleo sobre lienzo · 110 × 140 cm",
    price: "USD 32,400",
    tags: ["Validado por IA", "AR Disponible", "Custodia Lima"],
    palette: ["#1e2027", "#3d4a5c", "#7d8aa0", "#cfd5df"],
  },
  {
    lot: "Lote 042",
    title: "Cosecha en Maras",
    master: "Maestra Florinda Quispe",
    origin: "Sacred Valley · 2025",
    medium: "Óleo sobre lienzo · 95 × 130 cm",
    price: "USD 22,800",
    tags: ["Validado por IA", "Edición única", "AR Disponible"],
    palette: ["#2a1e15", "#8c5a2e", "#d49a4a", "#f1deb6"],
  },
];
    ...
    <section id="catalogo" className="bg-alt border-t border-b rule">
      <div className="max-w-[1320px] mx-auto px-6 lg:px-10 py-24 lg:py-32">
        ...
        <div className="grid md:grid-cols-3 gap-8 lg:gap-10">
          {WORKS.map((w, i) => <WorkCard key={w.lot} w={w} idx={i} />)}
        </div>
...
function FilterChip({ children, active }) {
  return (
    <button
      className={`font-display text-[11px] tracking-[0.18em] uppercase px-4 py-2 border rule rounded-full transition-colors ${
        active ? "" : "ink-soft hover:[color:var(--ink)]"
      }`}
  ...
  return (
    <article className="work-card group">
      <div className="work-mask aspect-[4/5] placeholder-canvas relative border rule">
        {/* color-blocked placeholder informed by palette */}
        ...
        <button className="absolute right-4 top-4 w-9 h-9 rounded-full border rule flex items-center justify-center"
...
function Footer() {
  const cols = [
    {
      h: "Plataforma",
      links: ["Catálogo curado", "Tecnología IA/AR", "Logística llave en mano", "Custodia y bóveda", "Reportes de procedencia"],
    },
    {
      h: "Compañía",
      links: ["Tesis institucional", "Equipo curatorial", "Maestros afiliados", "Notas de prensa", "Carreras"],
    },
    {
      h: "Coleccionistas",
      links: ["Acreditación KYC", "Mesa privada", "Servicios fiduciarios", "Donación filantrópica", "Atención 24/7"],
    },
  ];
...
function TweaksRoot() {
  ...
  useEffect(() => {
    ...
    root.style.setProperty("--accent-soft", t.accent);
  }, [t.accent]);
    ...
    <TweaksPanel title="Tweaks">
      ...
      <TweakSection label="Acento">
        <TweakColor
          ...
          options={["#8A6A3B", "#B08144", "#C9A86A", "#6E5638", "#1F1B16"]}
        />
      </TweakSection>
    </TweaksPanel>
  );
